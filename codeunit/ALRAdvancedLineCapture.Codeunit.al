...
codeunit 50110 "ALR Advanced Line Capture"
{
...
#pragma warning restore AA0237
    ...
    local procedure CDCCaptureEngine_OnBeforeAfterCapture(Document: Record "CDC Document"; var IsHandled: Boolean)
    var
        ...
        OnAfterStandardLineRecognitionHandled: Boolean;
    begin
        //CleanupPrevValues(Document); TODO => analyze if standard mess up document value table

        OnAfterStandardLineRecognition(Document, OnAfterStandardLineRecognitionHandled);
        if OnAfterStandardLineRecognitionHandled then
            exit;

        if not ModuleLicense.IsAdvCaptureActivated(false) then
            exit;

        if not CDCTemplate.Get(Document."Template No.") then
            exit;

        if (CDCTemplate."Recognize Lines" = CDCTemplate."Recognize Lines"::No) then
            exit;

        // Check if template is configured for ALR and exit if not
        if not TemplateIsConfiguredForAdvLineRecognition(CDCTemplate) then
            exit;
    ...
    local procedure TemplateIsConfiguredForAdvLineRecognition(Template: Record "CDC Template"): Boolean
    var
        CDCTemplateField: Record "CDC Template Field";
    begin
        CDCTemplateField.SetRange("Template No.", Template."No.");

        // Filter for fields that are configured for ALR recognition types
        CDCTemplateField.SetFilter("Advanced Line Recognition Type", '<>%1', CDCTemplateField."Advanced Line Recognition Type"::Default);
        if not CDCTemplateField.IsEmpty then
            exit(true);

        CDCTemplateField.SetRange("Advanced Line Recognition Type");

        // Filter for fields that are configured for replacement fields
        CDCTemplateField.SetFilter("Replacement Field", '<>%1', '');
        if not CDCTemplateField.IsEmpty then
            exit(true);

        CDCTemplateField.SetRange("Replacement Field");

        // Filter for fields that are configured to receive field value from previous lines field value
